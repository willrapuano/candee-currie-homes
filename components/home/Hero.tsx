'use client'

import Image from 'next/image'
import Link from 'next/link'

export function Hero() {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0">
        <Image
          src="https://images.unsplash.com/photo-1560179707-f14e90ef3623?w=1920&q=85"
          alt="Northern Virginia luxury homes — Arlington, McLean, Falls Church, Alexandria"
          fill
          priority
          sizes="100vw"
          className="object-cover object-center"
        />
        <div className="absolute inset-0 bg-hero-overlay" />
        <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-navy/40 to-transparent" />
      </div>

      <div className="relative container-xl pt-32 pb-24 flex flex-col items-center text-center">
        {/* Eyebrow */}
        <div className="flex items-center gap-3 mb-6">
          <div className="h-px w-10 bg-gold" />
          <span className="text-gold text-xs font-semibold tracking-[0.25em] uppercase">
            TTR Sotheby&apos;s International Realty
          </span>
          <div className="h-px w-10 bg-gold" />
        </div>

        {/* Headline */}
        <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl text-white font-bold leading-tight text-balance mb-4">
          Your Northern Virginia
          <br />
          <span className="text-gold">Real Estate Expert</span>
        </h1>

        {/* Subheadline */}
        <p className="text-white/80 text-lg md:text-xl max-w-2xl leading-relaxed mb-4">
          Candee Currie — Associate Broker at TTR Sotheby&apos;s. 14 years. 241 transactions. $105M+ volume.
          <br className="hidden md:block" />
          Arlington, McLean, Falls Church, Alexandria.
        </p>

        {/* Social proof micro-line */}
        <div className="flex items-center gap-4 mb-10 text-white/50 text-sm">
          <span>⭐ 5.0 across 40+ reviews</span>
          <span className="h-3 w-px bg-white/20" />
          <span>241 closed transactions</span>
          <span className="h-3 w-px bg-white/20" />
          <span>$105M+ volume</span>
        </div>

        {/* Primary CTAs — lead gen first, listings second */}
        <div className="flex flex-wrap justify-center gap-4">
          <Link href="/home-value" className="btn-gold text-base px-8 py-4">
            What&apos;s My Home Worth? →
          </Link>
          <Link href="/contact" className="btn-navy-outline text-base px-8 py-4">
            Talk to Candee
          </Link>
        </div>

        {/* Secondary nav */}
        <div className="flex flex-wrap justify-center gap-6 mt-8 text-xs text-white/50">
          <Link href="/neighborhoods" className="hover:text-gold transition-colors">Browse Neighborhoods</Link>
          <span className="h-3 w-px bg-white/20 self-center" />
          <Link href="/market" className="hover:text-gold transition-colors">NoVA Market Report</Link>
          <span className="h-3 w-px bg-white/20 self-center" />
          <Link href="/contact" className="hover:text-gold transition-colors">Work With Candee</Link>
        </div>
      </div>
    </section>
  )
}
